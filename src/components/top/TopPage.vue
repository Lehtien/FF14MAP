...
const getImage = (e) => {
  if (e.clipboardData.types[0] === 'Files') {
    ...
    similar_img.value = []
    axios.post(`http://127.0.0.1:8000/uploadfile/${select.value}`, fd
    ).then((response) => {
      const coordinates = response.data.coordinates;
      for (let place of coordinates) {
        coordinatesList.value.push(place[0])
      }
      console.log(coordinatesList.value)
